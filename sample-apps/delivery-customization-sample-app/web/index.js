...
const { SHOPIFY_HIDE_SHIPPING_METHODS_ID } = dotenv.parse(
  readFileSync(join(process.cwd(), "../", ".env"), "utf8")
);

const METAFIELD = {
  namespace: "sample-delivery-customization-hide-2",
  ...
  app.post("/api/delivery-customization", async (req, res) => {
    ...
    let query = {
      data: {
        ...
        variables: {
          deliveryCustomization: {
            functionId: SHOPIFY_HIDE_SHIPPING_METHODS_ID,
            title: `Hide "${payload.shippingMethod}" shipping methods if over threshold`,
            enabled: true,
          },
        },
      },
