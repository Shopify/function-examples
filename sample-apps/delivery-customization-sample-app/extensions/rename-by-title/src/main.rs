...
pub struct Configuration {
    pub titles_to_rename: Vec<String>,
}
...
fn function(input: Input) -> Result<FunctionResult, Box<dyn std::error::Error>> {
    ...
    let titles_to_rename = input.configuration().titles_to_rename;
    let operations = delivery_options
        .iter()
        .filter_map(|delivery_option| {
            if titles_to_rename.contains(&delivery_option.title) {
                Some(Operation {
...
mod tests {
    ...
    fn test_rename_operations_with_configuration() {
        let input = input(Some(Configuration {
            titles_to_rename: vec!["Method A".to_string(), "Method C".to_string()],
        }));
        let operations = function(input).unwrap().operations;

        assert_eq!(operations.len(), 2);
        assert_eq!(
            operations[0].rename.as_ref().unwrap().delivery_option_handle,
            "method-a"
        );
        assert_eq!(
        ...
        assert_eq!(
            operations[1].rename.as_ref().unwrap().delivery_option_handle,
            "method-c"
        );
        assert_eq!(
